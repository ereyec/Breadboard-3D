...
for(int i = 0; i < common.netlistVector.size(); i++){
	...
	if(std::strcmp(common.netlistVector[i][0].c_str(), "-i") == 0){ //User must manually add "-i" flag to ICs
...
for(int i = 0; i < common.netlistVector.size(); i++){
	...
	if(type == 'R'){
		...
		row = {R, {e1, e2}};
		//row.type = 99;
		//row.nodes.push_back(e1);
		//row.nodes.push_back(e2);
...
for(int i = 0; i < index + 1; i++){
	...
	for(int j = 0; j < common.elementTable.size(); j++){
		for(int k = 0; k < common.elementTable[j].nodes.size(); k++){
...
for(int i = 0; i < common.netlistVector.size(); i++){
	if(std::strcmp(common.netlistVector[i][0].c_str(), "-v") == 0){
